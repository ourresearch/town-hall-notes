...
| Shylaja L (Informatics) | License metadata mismatch — e.g. `10.1515/chem-2017-0001`: OpenAlex says CC-BY but the article is BY-NC-ND 3.0. | open | Already filed at support@ (no reply yet). Kyle asked for more examples; we prioritize bug fixes that come through support. **Action: chase the existing support ticket.** |
...
| Tijmen Altena | Should OpenAlex IDs be persistent? Since Walden there's no `merged_ids` list, so a merged author ID no longer resolves. | answered (on call) — doc follow-up open | Jason: doesn't need to be a true PID; transaction logs + redirects can be enough. Jason wants to understand the migrate-IDs-forward path better. **Action: document the post-Walden ID redirect/migration story.** |
