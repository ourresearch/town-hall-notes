...
| Country-scoped affiliation linking/unlinking in the org curation tool — let an org say "all `KTH`-from-Sweden strings → KTH Royal Inst of Tech" (Tobias Jeppsson) | not planned for now | — | Agreed this is valuable but hard to do well; we're not taking it on yet. The related, smaller piece (preview/export the affected works before a bulk link) has shipped — see next row. |
| Export the set of papers under a given affiliation before bulk link/unlink (not possible in the curation UI today) (Tobias Jeppsson) | done — shipped 2026-06-07 | — | Done. In the org curation tool, open any raw-affiliation string to preview the works it covers, then **Export CSV** to download the exact set a bulk link/unlink would touch (id, title, year, DOI, source) — xpac works included, so the count matches what gets linked. |
| Add users from outside your email domain to the org dashboard, for outside partners (Maria Praetzellis) | ✅ shipped | — | **Live now.** Org owners/admins can "Invite member" by email — including outside your org's domain — via an invite + accept handshake. |
| Collections available via the API (Maria Praetzellis) | ✅ done | — | **Works today** — query your collections and the `collection:` filter with your API key (`Authorization: Bearer <api_key>`). We just removed the old JWT requirement so this is clean for all clients, and corrected the docs guide. |
| Expose first-name + last-name from the new author-name recognition in the API (Tijmen Altena) | ✅ shipped (open-sourced) | — | We open-sourced the parser as **`whatsername`** (`pip install whatsername` · https://pypi.org/project/whatsername/): given a name it returns title / first / middle / last / suffix with a confidence score, ~90% accurate. |
